    ...
    try:
        ...
        for key in response['AccessKeyMetadata'] :
            if key['Status'] == "Active":
                if DryRun == False:
                    ...
                    print("DRYRUN Planned Action: User=%s removed AccessKey=%s" % (DeactivateUser, key['AccessKeyId']))
        ...
        for g in user_groups['Groups']:
            if DryRun == False:
                ...
                print("DRYRUN Planned Action: User=%s removed from Group=%s" % (DeactivateUser, g['GroupName']))
        ...
        for d in user_mfa_devices['MFADevices']:
            if DryRun == False:
                ...
                print("DRYRUN Planned Action: User=%s deactivated MFA=%s" % (DeactivateUser, d['SerialNumber']))
            if DryRun == False:
                response = iam_client.delete_virtual_mfa_device(SerialNumber=d['SerialNumber'])
                print("Removed MFA=%s" % (d['SerialNumber']))
            else:
                print("DRYRUN Planned Action: Removed MFA=%s" % (d['SerialNumber']))
        ...
        for i_policy in range(len(user_inline_policies['PolicyNames'])):
            if DryRun == False:
                ...
                print("DRYRUN Planned Action: User=%s removed inline policies=%s" % (DeactivateUser, user_inline_policies['PolicyNames'][i_policy]))
        ...
        for m_policy in user_managed_policies['AttachedPolicies']:
            ...
            if DryRun == False:
                ...
                print("DRYRUN Planned Action: User=%s removed managed policy=%s" % (DeactivateUser, m_policy['PolicyArn']))
        ...
        if DryRun == False:
            ...
            print("DRYRUN Planned Action: User:%s login profile deleted" % (DeactivateUser))
